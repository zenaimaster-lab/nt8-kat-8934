...
### [v1.01] — 2026-08-08
- **Re-audit Program + DailyRisk quick-sets — fix + improve + test + tách module**:
  - **Fix high**: `Draw.cs:1071` merge duplicate `btnDailyMaxDD/MaxProfit` click handlers (trước 2 handlers → nay single toggles + `UpdateDailyRiskPresetButtons/UpdateTradingProfileButtons` trong 1 handler); `HudFactory.cs:28` freeze `profileOffBg/profileRowOnBgs/dailyRiskPreset*` via `CreateFrozenBrush` (cross-thread); `Kat34Scalper.cs:886` add `AccountNameConverter` TypeConverter cho 8 `TradingProfileXAccount` (dropdown account list như TradeManager), `QuickSetLabelColor/ProgramLabelColor` simplify Freezable→SolidColorBrush only (remove `Freezable` using).
  - **Tách module**: new `src/Kat34Scalper.Profile.cs` partial class — move 13 helpers `GetTradingProfile* (8) / IsActive / UpdateTradingProfileButtons / ApplyTradingProfile / GetDailyRiskPreset* / Apply/UpdateDailyRiskPresetButtons` khỏi `Draw.cs` (Draw 1411→~950L, Profile 340L), `Draw.cs:450` placeholder comment, giữ fields `tradingProfileButtons/dailyRiskPresetButtons/activeTradingProfile/lastProfileApplyUtc` tại Draw, `Profile.cs` access via partial. Deploy/Ninja compile gate auto picks `src/*.cs`.
  - **Thêm tests**: `Kat34ScalperLogicTests.cs:1193` 4 tests `NormalizeProfileName_WithinEight_Kept / OverEight_Truncated / EmptyFallback / WhitespaceTrimmed` (8-char contract như `KatTradeCalculator.NormalizeProfileName`), total 124→128 pass.
  - **Improve**: `Draw.cs:1333` watchdog thêm `UpdateTradingProfileButtons/UpdateDailyRiskPresetButtons/UpdateAtmSetButtons` 500ms (sync highlight khi settings manual edit); `accComboBox` promotion field sync `ApplyTradingProfile` direct (bỏ visual-tree search); `Kat34ScalperLogic.cs:129` `NormalizeProfileName` doc + max8.
  - Verify: `Run-AllChecks` 3 steps 128 tests 0 warn ALL GREEN; `CompileCheck` net48 0 errors; `Deploy-NT8.ps1` OK; `graphify update` 825→~830 nodes.
  - Graphify mapping: `Kat34Scalper.Profile:GetTradingProfileName/.../IsActive/UpdateTradingProfileButtons/ApplyTradingProfile/GetDailyRiskPreset*/UpdateDailyRiskPresetButtons`, `Kat34ScalperLogic:NormalizeProfileName` (profile tests), `Kat34Scalper.Draw.HudFactory:profileOffBg/profileRowOnBgs` (frozen).
